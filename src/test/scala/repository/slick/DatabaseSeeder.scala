...
trait DatabaseSeeder {
  ...
  def createSchema()(implicit ec: ExecutionContext, db: Database): Future[Unit] = {
    db.run(DBIO.seq(
      sqlu"""
         CREATE TABLE IF NOT EXISTS orm_encapsulados.author (
         ...
         """,
      sqlu"""
         CREATE TABLE IF NOT EXISTS orm_encapsulados.post (
             id BIGINT AUTO_INCREMENT PRIMARY KEY,
             content TEXT NOT NULL,
             author_id BIGINT NOT NULL,
             FOREIGN KEY (author_id) REFERENCES orm_encapsulados.author(id)
         );
         """,
      sqlu"""
         CREATE TABLE IF NOT EXISTS orm_encapsulados.comment (
             id BIGINT AUTO_INCREMENT PRIMARY KEY,
             text TEXT NOT NULL,
             author_id BIGINT NOT NULL,
             ...
             FOREIGN KEY (author_id) REFERENCES orm_encapsulados.author(id),
             FOREIGN KEY (parent_comment_id) REFERENCES orm_encapsulados.comment(id),
             FOREIGN KEY (post_id) REFERENCES orm_encapsulados.post(id)
         );
         """
    ))
  }
